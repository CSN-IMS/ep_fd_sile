...
import config
import config_MCS_pynq

# python3 test_anal_fp.py mcs_number channel_name self_iterations
# ex:
# python3 test_anal_fp.py 1 proakisc 1

mcs_number = int(sys.argv[1])
channel_name = sys.argv[2]
S = int(sys.argv[3])

[N, Q, X_value, Kb, cstl, matcher_ind, dematcher_ind, cyc_shift, pynq_block] = config_MCS_pynq.get_mcs_config(mcs_number)
...
[chn_p, channel_h] = config_MCS_pynq.get_chn_config(channel_name, Ns*2+FS)
...
# b1["decode::v_d"] = v_d
b1["decode::x_d_F"] = pynq["decode_s1::x_d_F"]
b1["decode::v_d"] = pynq["decode_s1::v_d"]
pynq["decode_s0::x_e"] = b1["decode::x_e"]
pynq["decode_s1::x_e"] = b1["decode::x_e"]
pynq["decode_s0::v_e"] = b1["decode::v_e"]
pynq["decode_s1::v_e"] = b1["decode::v_e"]
rd["rate_dematch::x"] = pynq["decode_s0::Le"]
...
# SNR = np.ndarray(shape = b1["decode::SNR"][:].shape, dtype = np.float32)
# b1["decode::SNR"] = SNR
