...
# python3 test_anal.py mcs_number channel_name self_iterations
# ex:
# python3 test_anal.py 1 proakisc 1

mcs_number = int(sys.argv[1])
channel_name = sys.argv[2]
S = int(sys.argv[3])

[N, Q, X_value, Kb, k_PAM, cstl, matcher_ind, dematcher_ind, cyc_shift, B3_cons, B4_cons] = config_MCS.get_mcs_config(mcs_number)
...
b0 = B0.Block0(N, Q, k_PAM) # k_PAM has shown to degrade BPSK
b1 = B1.Block1(N, Q)
b3 = B3_cons(N, Q)
b4 = B4_cons(N, Q, ntanh)
b5 = B5_anal_fact.Block5(N, Q)
...
b3["decode::x_e"] = b1["decode::x_e"]
b3["decode::v_e"] = b1["decode::v_e"]
swi["commute::data "] = b3["decode::Le"]
rd["rate_dematch::x"] = swi["commute::data1"]
swi["select::data0"] = swi["commute::data0"]
cnt["iterate"] = swi["select::status"]
swi["commute::ctrl "] = cnt["iterate::out"]
b4["decode::Le"] = swi["select::data"]
b4["decode::v_e"] = b1["decode::v_e"]
b5["decode::x_e"] = b1["decode::x_e"]
b5["decode::v_e"] = b1["decode::v_e"]
b5["decode::mi_d"] = b4["decode::mi_d"]
b5["decode::Cep"] = b4["decode::Cep"]
...
for i in range(len(sigma_vals)):
	...
	while not mnt.is_done():
		...
		for s in range(S):
			...
			b4["decode"].exec()
			b5["decode"].exec()
			# print(np.abs(mdmout-b5.xstarout)>1e-5)
			# print(mdmout[0][48])
			# print(b5.xstarout[0][48])
			# time.sleep(1)
		b1["decode"].exec()
		b3["decode"].exec()
		swi["commute"].exec()
