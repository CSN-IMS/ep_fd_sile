...
class PynqConnect(Py_Module):
    def pynq_s0(self, x_e, v_e, Le):
        ...
        for frame in range(Frames):
            ...
            data_send = Fxp(ve, signed=False, n_word=8, n_frac=7, rounding="floor").raw().astype(np.uint8).tobytes() +np.array([0, 0, 64], dtype=np.uint8).tobytes()
            ...
            # buf_aux = np.empty((self.K*self.Q), dtype=np.int8)
            buf_aux = np.empty((self.K*self.Q), dtype=np.int8)
            buf_aux[::3] = buf_data[0::4]
            buf_aux[1::3] = buf_data[1::4]
            buf_aux[2::3] = buf_data[2::4]

            Le_d_kq = Fxp(buf_aux, raw=True, signed=True, n_word=8, n_frac=3, rounding="floor")
            ...
            # print(x_star.raw().astype(np.uint8))
            # time.sleep(1)
    ...
    def pynq_s1(self, x_e, v_e, x_d_F, v_d):
        ...
        for frame in range(Frames):
            ...
            # print("------------------------------------------------------")
            ve = v_e[frame][0]
            data_send = Fxp(ve, signed=False, n_word=8, n_frac=7, rounding="floor").raw().astype(np.uint8).tobytes()+np.array([0, 0, 192], dtype=np.uint8).tobytes()
            data_send = data_send + Fxp(x_e[frame], signed=True, n_word=8, n_frac=5, rounding="floor").raw().astype(np.uint8).tobytes()
            ...
            x_star = Fxp(buf_data[4:], raw=True, signed=True, n_word=8, n_frac=5, rounding="floor").get_val()
