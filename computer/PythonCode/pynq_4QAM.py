...
class PynqConnect(Py_Module):
    def pynq_s0(self, x_e, v_e, Le):
        ...
        for frame in range(Frames):
            ...
            data_send = Fxp(ve, signed=False, n_word=8, n_frac=7, rounding="floor").raw().astype(np.uint8).tobytes()+np.array([0, 0, 32], dtype=np.uint8).tobytes()
    ...
    def pynq_s1(self, x_e, v_e, x_d_F, v_d):
        ...
        for frame in range(Frames):
            ...
            # print("------------------------------------------------------")
            ve = v_e[frame][0]
            data_send = Fxp(ve, signed=False, n_word=8, n_frac=7, rounding="floor").raw().astype(np.uint8).tobytes()+np.array([0, 0, 160], dtype=np.uint8).tobytes()
            data_send = data_send + Fxp(x_e[frame], signed=True, n_word=8, n_frac=5, rounding="floor").raw().astype(np.uint8).tobytes()
