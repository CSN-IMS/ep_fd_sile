...
class PynqConnect(Py_Module):
    def pynq_s0(self, x_e, v_e, Le):
        ...
        for frame in range(Frames):
            # print(Fxp(v_e[frame], signed=False, n_word=8, n_frac=7).raw().astype(np.uint8))
            ...
            # send K_PAM * ve
            data_send = Fxp(2*ve, signed=False, n_word=8, n_frac=7, rounding="floor").raw().astype(np.uint8).tobytes()+np.array([0, 0, 0], dtype=np.uint8).tobytes()
            ...
            Le_d_kq = Fxp(buf_data[::2], raw=True, signed=True, n_word=8, n_frac=3, rounding="floor")
    ...
    def pynq_s1(self, x_e, v_e, x_d_F, v_d):
        ...
        for frame in range(Frames):
            ...
            # send K_PAM * ve
            data_send = Fxp(2*ve, signed=False, n_word=8, n_frac=7, rounding="floor").raw().astype(np.uint8).tobytes()+np.array([0, 0, 128], dtype=np.uint8).tobytes()
            data_send = data_send + Fxp(x_e[frame], signed=True, n_word=8, n_frac=5, rounding="floor").raw().astype(np.uint8).tobytes()
            ...
            #divide by K_PAM in the end
            v_d[frame] = v_star_fp/2
